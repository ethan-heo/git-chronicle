...
화면 문서는 단일 Feature로 구성된 화면(S07)의 경우 진입 조건·화면 상태·내비게이션 흐름처럼 해당 Feature의 `blueprint.md`에는 없는 화면 단위 정보를 담고, 여러 Feature가 조합되는 화면(S02)은 그 조합 관계 자체를 문서화한다. S01과 S06은 더 이상 독립 화면이 아니라 각각 S02 사이드바 섹션과 사이드바 로컬 설정 뷰로 흡수되었다.
