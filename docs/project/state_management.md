...
- `openWorkspaceTab`은 포커스된 leaf pane 안에서 동일 대상 탭 중복 생성을 막고, 새 탭 생성 또는 활성화 시 `selectedCommit`을 탭의 커밋과 동기화한다.
- `splitWorkspacePaneWithTab`은 드래그된 탭을 다른 leaf pane의 상/하/좌/우 드롭 존에 떨어뜨렸을 때 `paneTree`를 split node로 치환한다.
...
| `openWorkspaceTab(...)` | 포커스된 leaf pane에 탭을 추가하거나 기존 탭을 활성화하고, `selectedCommit`를 탭 커밋으로 동기화 |
| `activateWorkspaceTab(paneId, tabId)` | 대상 leaf pane의 `activeTabId`를 갱신하고 `focusedPaneId = paneId`로 맞춘다 |
| `focusPane(paneId)` | 패널 클릭 시 포커스를 이동하고, 사이드바 기준 `selectedCommit`를 해당 pane의 활성 탭 커밋으로 동기화한다 |
| `splitWorkspacePaneWithTab(...)` | 드래그된 탭을 분리해 새 leaf pane을 만들고 `paneTree`를 split node로 재구성한다 |
