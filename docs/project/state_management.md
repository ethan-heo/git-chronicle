...
| `openTabs` / `activeTabId` | S02 워크스페이스 본문 탭 상태. 열린 탭 목록과 활성 탭을 관리한다. |
...
- `openWorkspaceTab`은 동일 대상 탭 중복 생성을 막고, 새 탭 생성 또는 활성화 시 `selectedCommit`을 탭의 커밋과 동기화한다.
...
| `openWorkspaceTab({ panelType: 'code' })` | `selectedCommit`를 탭 커밋으로 갱신하고 `selectedFile`을 해당 경로에 맞게 동기화 |
| `openWorkspaceTab({ panelType: 'aiSummary' })` | `selectedCommit`를 탭 커밋으로 갱신하고 AI 정리 상태를 유지한 채 `aiSummary` 탭 활성화 |
| `openWorkspaceTab({ panelType: 'fileCanvas' })` | `selectedCommit`를 탭 커밋으로 갱신하고 `fileCanvas` 탭 활성화 |
| `openWorkspaceTab({ panelType: 'symbolGraph' })` | `selectedCommit`를 탭 커밋으로 갱신하고 `selectedFileForSymbolGraph`를 동기화 |
| `openWorkspaceTab({ panelType: 'note' })` | `selectedCommit`를 탭 커밋으로 갱신하고 `note` 탭 활성화 |
