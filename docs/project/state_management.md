...
| `currentSummaryContent`, `isLoadingSummary`, `isGeneratingSummary`, `summaryError`, `summarySavedPath`, `hasCurrentSavedSummary`, `isSummaryTokenLimitExceeded` | F05 |
...
- `selectFileForCode`/`selectFileForAI`는 이전 파일의 잔여 AI 요약 상태가 섞이지 않도록 `currentSummaryContent`, `isLoadingSummary`, `isGeneratingSummary`, `summaryError`, `summarySavedPath`, `hasCurrentSavedSummary`, `isSummaryTokenLimitExceeded`를 함께 초기화한다.
...
- `completeAISummary`는 `changedFiles`에서 현재 `selectedFile.path`와 일치하는 항목의 `hasSavedSummary`도 함께 `true`로 갱신한다 — 파일 트리의 "AI 요약됨" 뱃지가 별도 재조회 없이 즉시 갱신되는 이유다.
...
F08 시작 전 `activeAIProvider`와 `savePath`를 검증한다. 누락 시 `Toast`(error)를 표시하고 시작하지 않는다(정확한 문구는 F08 blueprint.md Error States 참고). 통과하면 선택 커밋, 변경 파일 목록, 활성 AI provider, 저장 경로를 `START_BATCH_AI_SUMMARY`로 Extension Host에 전달한다.
...
stateDiagram-v2
    [*] --> Idle
    Idle --> Running : startBatchAISummary()\n(검증 통과) → handleBatchStarted
    Running --> Running : handleBatchProgress()\n(파일별 진행, hasSavedSummary 갱신)
    Running --> Cancelling : handleBatchCancelling()
    Running --> Idle : handleBatchComplete()\n(Toast: 성공/실패 N개)
    Cancelling --> Idle : handleBatchCancelled()\n(Toast: 완료된 파일 수)
```

- `handleBatchComplete`는 `batchFailedCount > 0`이면 warning Toast, 아니면 success Toast를 띄운다. 정확한 문구는 F08 blueprint.md Error States를 따른다.
- 취소는 즉시 중단하지 않는다 — `isBatchCancelling`만 세우고, 현재 처리 중인 파일이 끝난 뒤 `handleBatchCancelled`로 전환된다.
...
| `selectFileForCode` | `summaryMode = 'file'`, `currentSummaryContent = ''`, `isLoadingSummary = false`, `isGeneratingSummary = false`, `summaryError = null`, `summarySavedPath = null`, `hasCurrentSavedSummary = file.hasSavedSummary`, `isSummaryTokenLimitExceeded = false`, `currentScreen = 'S03'`, 이전 화면 저장 |
