...
```typescript
// src/webview/store/useAppStore.ts
...
interface AppState {
  ...
  navigateBack: () => void;
  setGitRepoDetected: (detected: boolean) => void;
  setCommitList: (commits: Commit[], append?: boolean) => void;
  selectCommit: (commit: Commit) => void;
  resetCommitSelection: () => void;
  ...
  setAuthorList: (authors: string[]) => void;
...
커밋 선택 시 파일 관련 상태와 AI 정리 상태를 초기화한다.
...
| 트리거 | 초기화 대상 |
|--------|------------|
| `selectedCommit` 변경 | `selectedFile`, `changedFiles`, `currentSummaryContent`, `summaryError` |
| `isBatchRunning` → `false` | `batchTotal`, `batchCurrent`, `batchFailedCount`, `batchCancelled` |
| Extension 재활성화 | `currentScreen = 'S01'`, `previousScreen = null`. AI/저장경로는 `globalState`에서 복원 |
