...
`useAppStore`(`src/webview/store/appStore.ts`)는 11개 도메인 slice를 조합하는 combinator다. 각 slice는 `src/webview/store/slices/`에 독립 파일로 존재하며, slice 경계는 "화면/기능 전이 또는 단일 비동기 라이프사이클에 대한 응집도" 기준으로 나뉜다([coding_standards.md](./coding_standards.md)의 "레이어/모듈 구조 규칙" 참고). 실제 필드/액션 목록은 `ttsc_graph`로 조회한다 — 아래는 slice 파일과 담당 도메인 매핑만 표로 유지한다.
...
| `branchSlice.ts` | BranchSwitcher(F14) | 로컬 브랜치 목록, 최초 로드/refresh 상태, refresh 실패 Toast 규칙 관리 |
...
| `workspaceTabsSlice.ts` | WorkspaceTabs(F02/F11) | `paneTree`/`focusedPaneId`와 탭 열기·닫기·활성화·분할 액션, code 탭 내부 `codeInnerPaneTree` 자유 분할을 소유한다. `selectedCommit`은 commit이 있는 탭을 열 때만 갱신되고, 이미 열린 탭 사이의 포커스 전환에서는 유지된다 |
| `commitGroupSlice.ts` | CommitGroup(F13) | `commitGroups` 목록과 선택 모드 진행 상태(`isSelectModeActive`/`selectedCommitHashesForGroup`/`editingGroupId`)를 함께 관리. 그룹 필터 자체(`filterGroupId`)는 `commitListSlice.ts`의 `FilterState`에 속한다 |
...
- `filterBranch`가 있으면 Extension Host는 `git log <branch>` / `git rev-list --count <branch>` 범위로 조회한다. 단, F13 그룹 필터가 활성화된 요청은 이미 commit hash 집합을 직접 지정하므로 `filterBranch`를 전달하지 않는다.

### loadBranches / handleBranchesLoaded

| 액션 | 트리거 | 부수효과 |
|------|--------|----------|
| `loadBranches()` | S02 활성화 후 아직 브랜치 목록이 없을 때 | `isLoadingBranches = true` → `FETCH_BRANCHES` 전송 |
| `loadBranches({ refresh: true })` | F14 Fetch 버튼 클릭 | `isFetchingBranches = true` → 기존 목록 유지 → `FETCH_BRANCHES { refresh: true }` 전송 |
| `handleBranchesLoaded({ branches })` | 브랜치 응답 수신 | 목록 교체, 로딩 해제, 저장된 `filterBranch`가 없거나 삭제되었으면 HEAD 브랜치로 폴백 |
| `handleBranchesLoadFailed({ refresh: true })` | refresh 실패 | 기존 목록 유지, `pushToast(...)` 호출 |
...
- `openWorkspaceTab`은 포커스된 leaf pane 안에서 동일 대상 탭 중복 생성을 막고, 새 탭 생성 또는 기존 탭 재사용 활성화 시 사이드바 컨텍스트를 갱신한다. `selectedCommit`은 commit이 있는 탭을 열 때만 갱신한다. 탭 식별은 `panelType + commitHash + filePath`, `note:${relativePath}`를 각각 사용한다.
...
- `toggleCodeInnerPanel(paneId, tabId, panel)`은 `code` 탭의 `codeInnerPaneTree`만 갱신한다. 꺼져 있던 `aiSummary`/`symbolGraph`를 켤 때는 항상 `diff` 리프의 `right`에 새 split을 삽입하고, 켜져 있던 패널을 끌 때는 해당 리프를 제거한 뒤 형제 리프/서브트리가 부모 자리를 승계한다. `diff` 리프는 항상 정확히 1개 존재하며 닫을 수 없다.
- `moveCodeInnerPanel({ paneId, tabId, sourcePanel, targetPanel, zone })`은 같은 `code` 탭 안에서 `diff` / `aiSummary` / `symbolGraph` 리프를 상하좌우 가장자리 드롭으로 재배치한다. 최상위 `moveWorkspaceTab`과 같은 zone→orientation 매핑(`left|right → horizontal`, `top|bottom → vertical`)과 기본 `sizePercent: 50` 규칙을 따른다.
- `resizeCodeInnerSplit(paneId, tabId, nodeId, sizePercent)`은 `codeInnerPaneTree`의 split 노드 하나만 갱신한다. 리사이즈 상태도 Webview State에는 저장하지 않는다.
...
- `setAISummarySettings(...)`는 전달된 필드만 부분 병합한다(`savePath`/`registeredProviders`/`activeAIProvider`/`summaryModel`).
- `completeAISummary`는 `hasSavedCommitSummary`도 함께 `true`로 갱신한다 — 뒤로 돌아가도 저장본이 별도 재조회 없이 즉시 보이는 이유다.
- `setSummaryTokenWarning(isOverLimit)`은 `isSummaryTokenLimitExceeded` 플래그만 갱신하지만, `App.tsx`는 `AI_SUMMARY_TOKEN_WARNING` payload의 `commitHash`가 현재 `activeSummaryCommitHash`와 일치할 때만 이 액션을 호출한다. 늦게 도착한 이전 커밋 요청의 경고가 현재 다른 파일/탭 요약에 섞여 보이지 않게 하기 위한 가드다.

`App.tsx` 마운트 시 `FETCH_AI_SUMMARY_SETTINGS`로 Extension Host의 `globalState` 설정값을 먼저 복원한다. `activeAIProvider`와 `savePath`가 모두 있으면, 현재 컨텍스트가 커밋 전체면 `START_AI_SUMMARY_COMMIT`, 파일 트리에서 선택한 개별 파일이면 `START_AI_SUMMARY_FILE`을 보낸다. 설정 응답에는 `savePath`, `registeredProviders`, `activeAIProvider`, `summaryModel`이 포함된다.
...
요약이 완료된 뒤 사용자가 질문을 입력하면 `START_AI_QA`가 전송된다. Extension Host는 활성 프로바이더의 `summaryModel`로 응답을 생성하고, `AI_QA_CHUNK` / `AI_QA_COMPLETE` / `AI_QA_ERROR` 메시지로 상태를 전달한다. 완료 시 `currentSummaryContent`와 저장된 `.md` 파일이 동시에 갱신된다.

### F06/F07 설정 액션

F06 구현은 `REGISTER_AI_PROVIDER`로 CLI 버전 확인과 등록을 요청하고, 등록된 제공자는 `ACTIVATE_AI_PROVIDER`로 활성/비활성을 토글한다. 하나를 활성화하면 나머지는 자동으로 비활성 상태가 되며, 활성 버튼 하단에는 요약·Q&A 공통 모델 드롭다운이 노출된다. F05b는 `FETCH_AI_SUMMARY_SETTINGS` 응답의 `registeredProviders`, `activeAIProvider`, `savePath`, `summaryModel`을 `setAISummarySettings`에 반영한다.

모델 드롭다운 변경은 `SET_AI_MODEL` 메시지로 전달되며, Host는 `AI_MODEL_UPDATED` 응답으로 현재 활성 프로바이더의 `summaryModel`을 다시 내려준다.
...
| `gitRewind.summaryModelPerProvider` | `Record<AIProviderName, string>` | 현재 워크스페이스의 provider별 모델 (요약·Q&A 공통) |
...
| `workspaceState` | `gitChronicle.activeAIProvider`, `gitChronicle.savePath`, `gitRewind.summaryModelPerProvider` |

Extension 활성화 시 이 값을 읽어 Webview에 초기 상태로 전달한다.

F13 커밋 그룹은 별도로 `commitGroupService.ts`가 `workspaceState`에 `gitChronicle.commitGroups`(`CommitGroup[]`) 키로 저장한다. AI 설정과 달리 `globalState` fallback이나 VSCode configuration 연동은 없다.
...
현재 Webview State에 저장하는 값은 S01 필터, S02 사이드바 상태, S02 워크스페이스 탭 구조다. 탭 본문 스크롤 위치나 비동기 로딩 결과 자체는 저장하지 않고, 탭을 다시 열기 위한 최소 구조만 저장한다. 워크스페이스 탭 내부의 diff/AI 요약 스크롤 위치는 웹뷰 재생성 복원 대상은 아니지만, 같은 런타임 안에서 탭 전환 시 유지되도록 컴포넌트 바깥 `Map<cacheKey, scrollTop>` 캐시로 관리한다.
...
| `filter.filterBranch` | `string \| null` | 마지막 선택 브랜치 |
...
| `workspaceSidebar.*` | `PersistedWorkspaceSidebarState` | S02 사이드바 폭, 브랜치/커밋/파일/노트/PR/Issue 섹션 펼침 상태, 세로 분할 높이 |
| `workspaceTabs.paneTree` | `PaneNode` | S02 본문의 split 트리 구조, leaf pane별 `tabs`, `activeTabId`, code 탭 내부 `codeInnerPaneTree` |
| `workspaceTabs.focusedPaneId` | `string` | 마지막으로 포커스된 leaf pane id. 저장된 pane이 사라졌으면 첫 leaf pane으로 폴백 |
...
| code 탭 내부 diff/AI 요약 `scrollTop` | Webview State나 Zustand에는 저장하지 않는다. 같은 런타임 안의 탭 전환에서만 `workspace-tab:{tabId}:{panel}` 키 기반 메모리 캐시로 복원하고, 웹뷰가 파괴되면 함께 초기화된다 |
...
| S02에서 여러 탭/분할 열기 → VSCode 다른 탭 전환 → 패널 복귀 | `paneTree`, `focusedPaneId`, `codeInnerPaneTree`가 복원되고 각 탭 콘텐츠는 기존 mount 로직으로 재로드 |
...
| `openWorkspaceTab(...)` | 포커스된 leaf pane에 탭을 추가하거나 기존 탭을 활성화하고, 사이드바 커밋 컨텍스트(`selectedCommit`)를 새로 연 대상 기준으로만 갱신한다 |
...
| Extension 재활성화 | `currentScreen = 'S02'`. AI/저장경로는 `globalState`/`workspaceState`에서 복원, S01 필터와 S02 사이드바/워크스페이스 탭 구조는 Webview State에서 복원 |
