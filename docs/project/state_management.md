...
현재 Webview State에 저장하는 값은 S01 필터와 S02 사이드바 상태뿐이다. S02 split 레이아웃은 저장하지 않는다. 대신 워크스페이스 탭 내부의 diff/AI 요약 스크롤 위치는 웹뷰 재생성 복원 대상은 아니지만, 같은 런타임 안에서 탭 전환 시 유지되도록 컴포넌트 바깥 `Map<cacheKey, scrollTop>` 캐시로 관리한다.
...
| `paneTree` / `focusedPaneId` / `codeInnerPaneTree` | split layout은 재생성 비용보다 일관된 초기화가 중요하므로 매번 단일 pane으로 시작한다. code 탭 내부는 항상 diff 단일 리프로 리셋된다 |
