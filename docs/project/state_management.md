...
loadCommits: (reset = false) => {
  ...
  if (state.isLoadingCommits) {
    if (reset) set({ hasPendingCommitReload: true });
    return;
  }
...
handleCommitsLoaded: ({ commits, page, pageSize, hasMore, requestId }) => set((state) => {
  if (requestId !== undefined && state.pendingRequestId !== requestId) {
    return state;
  }
...
정렬 순서가 `asc`일 때는 Extension Host가 전체 커밋 수를 먼저 계산한 뒤 역산된 페이지 오프셋으로 오래된 순 목록을 가져와 응답한다. `hasMoreCommits`는 결과 행 수가 아니라 별도 `hasMore` 판정값으로 결정한다.
