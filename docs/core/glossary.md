...
| AI Summary (AI 정리) | AI CLI로 생성한 마크다운 요약. 커밋 단위(F05b)와 파일 단위 두 스코프가 있고, 노트로 저장할 때는 `.ai.md` 확장자를 강제한다 | `currentSummaryContent`, `START_AI_SUMMARY_COMMIT`/`START_AI_SUMMARY_FILE` (`src/extension/messageHandler.ts`) | F02, F05b, F09, F11 |
...
| Workspace Tab | S02 화면 본문에서 동시에 열어둘 수 있는 콘텐츠 탭(`code`/`aiSummary`/`fileCanvas`/`note`). `note` 탭은 커밋과 독립적으로 열리고, 파일 AI 요약과 심볼 캔버스는 `code` 탭 내부 패널로 연다 | `paneTree`, `WorkspaceTab`, `workspaceTabsSlice.ts` | F02, F03, F04, F05b, F09, F10, F11 |
