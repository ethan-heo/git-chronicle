...
## 6. User-Driven Analysis (사용자 주도 분석)

- AI와 시각화 도구는 분석 과정의 보조자이며, 분석의 주체는 항상 사용자다. 사용자가 명시적으로 요청하지 않은 범위까지 AI가 선제적으로 처리하지 않는다.
- 커밋 단위 요약(F05b)은 전체 맥락을 빠르게 파악하는 진입점으로 제공하되, 개별 파일에 대한 깊은 이해는 사용자가 직접 질문(F09 Q&A)하며 탐색하도록 유도한다.
- 파일 단위로 사전에 대량 생성되는 자동 요약(예: 전체 파일 일괄 정리)은 두지 않는다 — 사용자가 무엇을 더 볼지 매번 직접 선택하게 한다.
