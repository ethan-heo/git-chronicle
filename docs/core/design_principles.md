...
- 커밋 단위 요약(F05b)은 전체 맥락을 빠르게 파악하는 진입점으로 제공하고, 파일 트리의 파일 AI 액션은 사용자가 선택한 개별 파일만 즉시 요약한다.
- 파일 단위 자동 요약은 사용자가 명시적으로 연 파일에 한해서만 생성한다. 선택하지 않은 파일까지 선제적으로 일괄 생성하지 않는다.
