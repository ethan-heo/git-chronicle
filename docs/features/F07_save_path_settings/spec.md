...
|------|------|
| 설정 방법 | 디렉토리 선택 다이얼로그 |
| 저장 구조 | 파일 단위/커밋 단위 AI 요약 저장본은 사용자가 지정한 `{savePath}` 하위 노트 경로에 저장하되 실제 확장자는 항상 `.ai.md`다. 독립 노트(F11)는 `{savePath}` 아래 실제 디렉토리 구조를 그대로 사용하며 일반 `.md` 규칙을 유지한다 |
...
| 대상 | 저장 경로 |
|------|-----------|
| 파일 단위 AI 정리 | 사용자가 [저장] 시 선택한 `{savePath}` 하위 노트 경로 (`.ai.md` 강제) |
| 커밋 단위 AI 정리 | 사용자가 [저장] 시 선택한 `{savePath}` 하위 노트 경로 (`.ai.md` 강제) |
| 독립 노트(F11) | `{savePath}/{relativePath}` |

이전 버전의 AI 요약 자동 저장은 `normalizedFilePath`(파일 경로의 `/` 또는 `\`를 `__`로 치환한 값)와 커밋 해시 디렉토리 규칙을 사용했지만, 현재 버전은 기본 제안값 없이 사용자가 노트 경로를 직접 정한다.

**커밋 정리 파일명은 `vscode.env.language`에 따라 분기한다** (저장 시점 기준, `ko`로 시작하면 한국어·그 외는 영어):

| 언어 | 커밋 정리 파일명 |
|------|-----------------|
| 한국어 (`ko*`) | `전체_파일_정리.md` |
| 그 외 | `full_file_summary.md` |

예 (한국어 환경에서 저장한 경우):

```
{savePath}/ideas/커밋요약.ai.md
{savePath}/ideas/App요약.ai.md
{savePath}/ideas/todo.md
```
...
독립 노트(F11)와 AI 요약 저장본은 모두 `{savePath}` 하위의 실제 상대 경로를 그대로 사용한다. 다만 AI 요약 저장본은 새로 저장되는 시점부터 `.ai.md` 규칙을 적용하고, 기존 `.md` 저장본은 자동 마이그레이션하지 않는다.
