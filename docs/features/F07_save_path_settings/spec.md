...
| 커밋 노트 | `{savePath}/{shortHash}_{sanitizedCommitMessage}/노트.md` |
...
- 경로 영속 저장은 `src/extension/aiProviderService.ts`의 `setSavePath()`가 `gitChronicle.savePath` 키로 `ExtensionContext.workspaceState`에 저장한다.
...
- Webview는 `FETCH_CHANGED_FILES`, `START_AI_SUMMARY_COMMIT`, `START_AI_SUMMARY_FILE`, `START_AI_QA` 메시지에 현재 선택 커밋의 `commitMessage`를 포함해 Extension Host로 전달한다. 파일 컨텍스트에서는 `filePath`도 함께 보낸다.
