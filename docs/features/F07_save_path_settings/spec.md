...
|------|------|
| 설정 방법 | 디렉토리 선택 다이얼로그 |
| 저장 구조 | 파일 단위 `{루트경로}/{shortHash}_{sanitizedCommitMessage}/{파일명}.md`, 커밋 단위 `{루트경로}/{shortHash}_{sanitizedCommitMessage}/{커밋 정리 파일명}`. 독립 노트(F11)는 `{루트경로}/notes/` 아래 실제 디렉토리 구조를 그대로 사용한다 |
...
| 대상 | 저장 경로 |
|------|-----------|
| 파일 단위 AI 정리 | 사용자가 [저장] 시 선택한 `{savePath}` 하위 노트 경로 |
| 커밋 단위 AI 정리 | 사용자가 [저장] 시 선택한 `{savePath}` 하위 노트 경로 |
...
```
{savePath}/ideas/커밋요약.md
{savePath}/ideas/App요약.md
...
독립 노트(F11)와 AI 요약 저장본은 모두 `{savePath}` 하위의 실제 상대 경로를 그대로 사용한다. 기존 커밋 디렉토리 안의 파일은 그대로 남으며, 새 버전이 자동으로 마이그레이션하지 않는다.
