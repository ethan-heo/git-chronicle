...
커밋 요약만 보는 데서 끝나지 않고, 개별 파일을 포함한 변경 의도나 구현 배경을 직접 질문해 바로 답을 확인한다. 파일 단위로 미리 생성된 요약이 없는 대신, 궁금한 파일이 있으면 이 화면에서 질문으로 파고든다.
...
| 항목 | 내용 |
|------|------|
| 적용 범위 | F05b 커밋 단위 요약. 개별 파일에 대한 질문도 이 화면에서 커밋 전체 diff를 근거로 답변한다 |
...
| 메시지 | 방향 | 페이로드 |
|--------|------|---------|
| `START_AI_QA` | Webview → Host | `{ question, summaryContent, commitHash, commitMessage, provider, qaModel, savePath }` |
