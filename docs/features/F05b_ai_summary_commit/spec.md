...
| 용어 | 정의 | 관련 코드 식별자 |
|---|---|---|
| 저장본(Saved Summary) | 사용자가 [저장]을 눌러 노트로 저장한 AI 정리 결과. 링크 레코드에 `commitMessage`까지 함께 저장되며, 같은 커밋/파일 스코프로 다시 열면 연결된 노트를 즉시 불러온다 | `summaryNoteRelativePath`, `AI_SUMMARY_NOTE_LINKED` |
...
3. **저장본이 없는 경우**: 클릭 즉시 커밋 내 전체 파일 diff를 컨텍스트로 AI가 마크다운 형식으로 정리한다.
   ...
   - [저장]을 누르면 저장 버튼 바로 아래 팝오버가 열리고, 경로 입력창에서 입력한 키워드를 포함하는 기존 폴더 경로 목록을 보고 선택해 재사용하여 `.ai.md` 노트 파일로 저장한다.
...
| 재생성 | 재생성 아이콘 클릭 → 확인 다이얼로그 → 확인 시 동일 입력으로 재처리. 이전에 저장한 노트가 있었다면 재저장 팝오버에 그 경로를 기본값으로 채운다 |
...
| Mermaid preview | ```` ```mermaid ```` 코드블록은 다이어그램 preview로 렌더링되며, preview 상태에서도 hover 복사 버튼 또는 preview 선택 복사로 원본 Mermaid 마크다운 블록 복사가 가능해야 하고, 복사 버튼 클릭 시 성공 토스트를 표시한다. 렌더된 preview는 마우스 휠 줌(0.3x~2.0x), 빈 영역 드래그 팬, 우상단 `+ / - / fit` 버튼을 지원한다 |
| 토큰 사용량 표시 | Claude·Codex로 새로 생성이 완료되면 헤더에 `입력 N / 출력 M` 사용량을 표시한다. Claude는 비용(`$x.xxxx`)도 함께 표시하고, Gemini·저장본 즉시 로드·Q&A에는 표시하지 않는다 |
| 토큰 한계 초과 | 커밋 단위는 필터링된 diff 길이를 기준으로 "diff가 큽니다. AI가 일부를 생략할 수 있습니다" 경고 토스트를 표시한 뒤 호출한다. 이 경고는 `commitHash`가 현재 활성 요약 대상과 일치할 때만 반영되며, 이전 요청의 지연 응답이 다른 파일/탭 요약에 섞여 표시되면 안 된다 |
...
| 저장 파일 | `.ai.md` | 사용자가 선택한 `{savePath}` 하위 상대 경로에 AI 요약 노트 파일로 생성 또는 갱신 |
