...
| Code block copy | fenced code block 위에 hover 시 복사 버튼이 나타나며, 클릭 시 해당 코드블록의 원본 마크다운(```` 포함)을 클립보드에 기록한다 |
| Mermaid preview | ```` ```mermaid ```` 코드블록은 다이어그램 preview로 렌더링되며, preview 상태에서도 hover 복사 버튼 또는 preview 선택 복사로 원본 Mermaid 마크다운 블록 복사가 가능해야 한다 |
