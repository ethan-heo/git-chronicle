...
|------|------------|
| 파일 단위 (F05) | `{파일명}.md` |
| 커밋 단위 (F05b) | `_commit_summary.md` |

---

## Component Tree

F05_AISummaryFile의 Component Tree와 동일. `summaryMode = "commit"`으로 분기.

```
F05b_AISummaryCommit
...
[F05_AISummaryFile blueprint](../F05_ai_summary_file/blueprint.md#mcp-optimization-rules)와 동일. 별도 Frame 불필요 — S04 화면의 `summaryMode` Variant로 처리.
