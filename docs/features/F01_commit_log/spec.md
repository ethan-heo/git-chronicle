...
내가 작업한 특정 커밋을 날짜, 작성자, 메시지 또는 커밋 해시 키워드 기준으로 빠르게 찾아 같은 워크스페이스 안에서 후속 탐색으로 이어간다.
...
3. 필터 영역에서 다음 기준으로 목록을 필터링할 수 있다.
   ...
   - **커밋 메시지/해시 포함 키워드:** 텍스트 입력창에 키워드 입력 후 300ms 디바운싱
   - **커밋 메시지 제외 키워드:** 쉼표로 구분한 키워드를 입력하면 해당 문자열을 포함한 커밋을 제외
   - 포함 키워드는 커밋 메시지와 커밋 해시(short/full hash)에 대해 대소문자를 구분하지 않는다.
...
| 키워드 검색 | 커밋 메시지와 커밋 해시(short/full hash) 대상. 입력 후 300ms 디바운싱. 대소문자 구분 없음 |
