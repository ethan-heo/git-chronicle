...
- `selectedFile: ChangedFile` — 노드 클릭 시 전역 상태 업데이트 → S-03 진입
...
| 인터랙션 | 트리거 | 결과 |
|---------|--------|------|
| 노드 호버 | `FileNode` 마우스 진입 | `FileActionButtons` 표시 + 연결 엣지 강조 + 비연결 엣지 감쇠 |
| 노드 호버 해제 | 마우스 이탈 | `FileActionButtons` 숨김 + 엣지 강조 해제 + 감쇠 해제 |
...
| S03 뒤로가기 | `BackButton` 클릭 | `previousScreen`이 S05이면 S05 복귀 |
