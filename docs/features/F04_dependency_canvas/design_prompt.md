...
커밋에서 변경된 파일들을 노드로, 파일 간 import/require 의존 관계를 엣지(화살표)로 시각화하는 캔버스 화면을 디자인한다. 노드에 호버 시 [코드 보기]/[AI 정리 보기] 버튼이 나타난다. 전체 화면을 캔버스가 채우며, 범례 패널과 줌 컨트롤은 오버레이로 고정 배치된다.
...
- `noAnalysis`: JS/TS 외 파일 (툴팁 "의존 관계 분석 불가", 엣지 없음)
