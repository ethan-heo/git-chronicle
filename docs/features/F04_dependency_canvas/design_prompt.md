...
커밋에서 변경된 파일들을 노드로, 파일 간 의존 관계를 엣지(화살표)로 시각화하는 캔버스 화면을 디자인한다. JS/TS/CJS/ESM은 `dist/depcruiser-runner.mjs`를 통한 dependency-cruiser API 실행, Python/Go는 텍스트 파싱 기반으로 분석된다. 노드에 호버 시 [코드 보기]/[AI 정리 보기] 버튼이 나타난다. 전체 화면을 캔버스가 채우며, 범례 패널과 줌 컨트롤은 오버레이로 고정 배치된다.
