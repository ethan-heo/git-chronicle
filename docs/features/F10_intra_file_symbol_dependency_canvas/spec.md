...
5. 노드에 마우스를 호버링하면 연결된 엣지가 강조되고 비연결 엣지는 감쇠된다.
   - **[복사]** → 해당 심볼 노드가 의존하는 대상 심볼/엣지만 Mermaid 마크다운으로 복사
6. 헤더의 `[코드 보기]` 버튼을 누르면 오른쪽 코드 패널이 슬라이드 인한다.
7. 코드 패널에서 노드를 클릭하면 해당 라인 범위로 스크롤되고, 호버하면 강조만 갱신되며 스크롤은 발생하지 않는다.
8. 워크스페이스 좌측 `BackButton`을 클릭하면 S01로 복귀하고, 사이드바 설정 뷰를 열었다 닫아도 기존 S02 워크스페이스 문맥을 유지한다.
...
| 인터랙션 | 노드 호버 → 파일 캔버스와 동일한 위치의 [복사] 아이콘 노출 + 연결 엣지 강조 + 비연결 엣지 감쇠. 복사 시 Mermaid 노드 ID는 원본 소스 ID 대신 심볼명 기반 식별자를 사용하고, 호버한 심볼에서 바깥으로 나가는 직접 의존 심볼/엣지만 포함한다. 노드 드래그로 위치 조정 가능 |
...
| 코드 패널 복사 | 우측 코드 패널에서 라인 범위를 드래그로 선택하면 종료 지점 근처에 복사 아이콘이 나타나고, 클릭 시 선택한 코드 범위를 마크다운 코드블록으로 복사한다 |
...
| 다국어 | `symbolGraph` 패널의 로딩/빈 상태, 헤더의 심볼 개수 문구, 코드 패널, 범례 패널, 코드 뷰어 접근성 라벨은 `src/webview/i18n/locales/en/translation.json` 및 `ko/translation.json`의 `symbol_graph.*` 키로 관리한다 |
...
- [S02_WorkspaceScreen](../../screens/S02_history_view/blueprint.md) — 본문 `symbolGraph` 패널로 통합
...
| 심볼 그래프 탭 활성화 | `[심볼 그래프]` 버튼 클릭 | `openWorkspaceTab({ panelType: "symbolGraph" })` |
