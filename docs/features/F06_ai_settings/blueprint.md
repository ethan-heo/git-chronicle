...
Claude/Gemini/Codex CLI의 등록·활성화·비활성화를 관리하고, 활성 프로바이더별 모델을 선택하는 UI를 제공한다. 선택한 모델은 요약 생성과 Q&A 응답 생성에 공통으로 사용된다.
...
#### Purpose
활성 AI 프로바이더에 대해 요약 생성과 Q&A 응답 생성에 공통으로 쓰일 모델을 선택하는 드롭다운 그룹.
...
| 모델 선택 | 드롭다운 변경 | 활성 프로바이더의 `summaryModel` 갱신 |
