...
| 기본 모델 | Claude: `claude-haiku-4-5`, Gemini: `gemini-2.0-flash-lite`, Codex: `gpt-4o-mini` |
| 모델 영속화 | 프로바이더별 마지막 선택값을 `ExtensionContext.globalState`에 저장 |
...
| `summaryModel` | `string \| null` | 활성 프로바이더의 요약용 모델 |
...
| `summaryModel` | `string \| null` | 전역 상태 업데이트 |
| `qaModel` | `string \| null` | 전역 상태 업데이트 |
...
| `summaryModel` / `qaModel` 전역 상태 업데이트 | 모델 변경 | 활성 프로바이더에 대한 선택 모델 변경 |
