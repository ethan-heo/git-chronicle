...
- 접힌 Mermaid 위젯과 F05b의 `MermaidBlock` preview는 동일한 pan/zoom 헬퍼를 공유한다. 둘 다 마우스 휠 줌(0.3x~2.0x), 빈 영역 드래그 팬, 우상단 `+ / - / fit` 버튼을 제공하고, F11 위젯은 커서가 다시 블록 밖으로 나가 재마운트될 때마다 fit 기준 뷰로 시작한다
