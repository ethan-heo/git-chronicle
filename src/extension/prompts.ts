...
- Inside any Mermaid node or edge label, always wrap the label text in double quotes and keep it plain text — no backticks, no \`**bold**\`/\`*italic*\`, no other Markdown formatting inside labels (e.g. write \`C["Number(query[0]) 변환"]\`, not \`C[Number(query[0]) 변환]\` or \`C["\`Number(query[0]) 변환\`"]\`) — unquoted or Markdown-formatted labels break Mermaid's parser
